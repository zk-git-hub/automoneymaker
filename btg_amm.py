...
print("BTG-autotradestart")
# 시작 메세지 슬랙 전송
post_message(myToken,"#amm", "BTG-autotradestart")
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=1200):
            target_price = get_target_price("KRW-BTG", 0.3)
            btc = get_balance("BTG")
            krw = get_balance("KRW")
            ...
            if target_price < current_price and krw > 5*btc*current_price and target_price*1.15 > current_price:             
                krw = get_balance("KRW")
                if krw > 5000:
                    buy_result = upbit.buy_market_order("KRW-BTG", krw*0.3)
                    post_message(myToken,"#amm", "BTG buy : " +str(buy_result))
            if target_price*1.18 < current_price:
                btc = get_balance("BTG")
                krw = get_balance("KRW")
                current_price = get_current_price("KRW-BTG")
                if btc*current_price > 200000:
                    sell_result = upbit.sell_market_order("KRW-BTG", btc*0.25)
                    post_message(myToken,"#amm", "BTG buy : " +str(sell_result))
        else:
            btc = get_balance("BTG")
            current_price = get_current_price("KRW-BTG")
