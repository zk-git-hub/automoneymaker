...
print("DOGE-autotradestart")
# 시작 메세지 슬랙 전송
post_message(myToken,"#amm", "DOGE-autotradestart")
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=120):
            ...
            yesterday_price = get_yesterday_price("KRW-DOGE")
            ...
            if target_price*1.20 < current_price:
                if btc*current_price > 100000:
                    sell_result = upbit.sell_market_order("KRW-DOGE", btc*0.25)
                    post_message(myToken,"#amm", "DOGE sell: 익절 흐믓")

            #떨어진거 줍줍 한탕 노리자 (x%~18% 떨어지면 100,000 추가 매수)
            if current_price < yesterday_price*0.94 and current_price > yesterday_price*0.82 and 10000 > btc*current_price:
                if krw > 101000:
    ...
    #에러메세지 출력
        ...
        print(e)
        post_message(myToken,"#amm", "DOGE error : " +e)
        time.sleep(1)