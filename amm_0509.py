...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=200):
            ...
            target_price_eos = get_target_price("KRW-EOS", 0.2)
            print ("targetprice1")
            time.sleep(0.5)
            target_price_doge = get_target_price("KRW-DOGE", 0.2)
            target_price_xrp = get_target_price("KRW-XRP", 0.2)
            target_price_ada = get_target_price("KRW-ADA", 0.3)
            print ("targetprice2")
            time.sleep(0.5)
            target_price_bat = get_target_price("KRW-BAT", 0.1)
            target_price_ltc = get_target_price("KRW-LTC", 0.1)
            target_price_pci = get_target_price("KRW-PCI", 0.2)
            print ("targetprice3")
            time.sleep(0.5)
            target_price_vet = get_target_price("KRW-VET", 0.1)
            target_price_etc = get_target_price("KRW-ETC", 0.1)
            target_price_link = get_target_price("KRW-LINK", 0.1)
            print ("targetprice4")
            time.sleep(0.5)
            target_price_btg = get_target_price("KRW-BTG", 0.1)
