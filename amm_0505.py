...
time.sleep(0.2)
post_message(myToken,"#amm", "  웨이브-WAVES")
time.sleep(0.2)
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=200):
            ...
            target_price_waves = get_target_price("KRW-WAVES", 0.4)
            ...
            current_price_waves = get_current_price("KRW-WAVES")
            yesterday_price_waves = get_yesterday_price("KRW-WAVES")
            ...
            print ("buy8")
            if target_price_waves < current_price_waves and 10000 > waves*current_price_waves:
                if krw > 500500:
                    buy_result = upbit.buy_market_order("KRW-WAVES", 500000)
                    post_message(myToken,"#amm", "WAVES buy : 타겟 구매")
            time.sleep(0.5)
            ...
            print ("sell8")
            if target_price_waves*1.20 < current_price_waves:
                if waves*current_price_waves > 200000:
                    sell_result = upbit.sell_market_order("KRW-WAVES", waves*0.25)
                    post_message(myToken,"#amm", "WAVES sell: 익절 흐믓")
            time.sleep(0.5)
            ...
            print ("gazua8")
            if current_price_waves < yesterday_price_waves*0.90 and current_price_waves > yesterday_price_waves*0.82 and 10000 > waves*current_price_waves:
                if krw > 200100:
                    buy_result = upbit.buy_market_order("KRW-WAVES", 200000)
                    post_message(myToken,"#amm", "WAVES buy : 가즈아 구매")
            time.sleep(0.5)
            ...
            print ("mang8")
            if current_price_waves < yesterday_price_waves*0.8:
                if waves*current_price_waves > 5000:
                    sell_result = upbit.sell_market_order("KRW-WAVES", waves*0.9995)
                    post_message(myToken,"#amm", "WAVES sell all: 망했어요")
            time.sleep(0.5)
        ...
        #장마감 - 가진거 다팔아
            ...
            if waves*current_price_waves > 5000:
                sell_result = upbit.sell_market_order("KRW-WAVES", waves*0.9995)
                post_message(myToken,"#amm", "WAVES sell all : 장마감")
