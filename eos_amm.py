...
print("EOS-autotrade start")
# 시작 메세지 슬랙 전송
post_message(myToken,"#amm", "EOS-autotrade start")
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=600):
            target_price = get_target_price("KRW-EOS", 0.8)
            btc = get_balance("EOS")
            krw = get_balance("KRW")
            ...
            if target_price < current_price and krw > btc*current_price:             
                krw = get_balance("KRW")
                if krw > 5000:
                    buy_result = upbit.buy_market_order("KRW-EOS", krw*0.5)
