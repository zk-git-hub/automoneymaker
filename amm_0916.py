...
post_message(myToken,"#amm", "  비트코인-BTC // 이더리움-ETH // 비캐ABC-BCHA")
time.sleep(0.3)
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=200):
            ...
            target_price_btc = get_target_price("KRW-BTC", 0.1)
            target_price_eth = get_target_price("KRW-ETH", 0.7)
            target_price_bcha = get_target_price("KRW-BCHA", 0.5)
            print ("targetprice1")
            time.sleep(0.5)
            target_price_ltc = get_target_price("KRW-LTC", 0.2)
            target_price_etc = get_target_price("KRW-ETC", 0.8)
            target_price_dot = get_target_price("KRW-DOT", 0.1)
            print ("targetprice2")
            time.sleep(0.5)
            target_price_link = get_target_price("KRW-LINK", 0.2)
            target_price_ada = get_target_price("KRW-ADA", 0.4)
            target_price_xrp = get_target_price("KRW-XRP", 0.3)
            print ("targetprice3")
            time.sleep(0.5)
            ...
            yesterday_price_bcha = get_yesterday_price("KRW-BCHA")
            print ("price1")
            time.sleep(0.5)
            ...
            print ("buy 1-3")
            if target_price_bcha < current_price_bcha and 10000 > bcha*current_price_bcha:
                if krw > 200500 and yesterday_price_bcha*1.1 > current_price_bcha:
                    buy_result = upbit.buy_market_order("KRW-BCHA", 200000)
                    post_message(myToken,"#amm", "BCHA buy: 타겟가격 돌파_200,000")
            time.sleep(0.5)
            ...
            print ("gazua 1-3")
            if current_price_bcha < yesterday_price_bcha*0.90 and current_price_bcha > yesterday_price_bcha*0.82 and 200000 > bcha*current_price_bcha:
                if krw > 200100:
                    buy_result = upbit.buy_market_order("KRW-BCHA", 200000)
                    post_message(myToken,"#amm", "BCHA buy: 종가-10%_200,000")
            time.sleep(0.5)
            ...
            print ("sonjul 1-3")            
            if current_price_bcha < target_price_bcha*0.95:
                if bcha*current_price_bcha > 160000:
                    sell_result = upbit.sell_market_order("KRW-BCHA", bcha*0.75)
                    post_message(myToken,"#amm", "BCHA sell: 손실 관리_75% 매도")
            time.sleep(0.5)
            ...
            print ("sonjul 2-3")            
            if current_price_dot < target_price_dot*0.95:
            ...
            print ("mang 1-3")            
            if current_price_bcha < yesterday_price_bcha*0.8:
                if bcha*current_price_bcha > 5000:
                    sell_result = upbit.sell_market_order("KRW-BCHA", bcha*0.9995)
                    post_message(myToken,"#amm", "BCHA sell: 종가-20%_전체매도")
            time.sleep(0.5)
        ...
        #장마감 - 가진거 다팔아
            ...
            if bcha*current_price_bcha > 5000:
                sell_result = upbit.sell_market_order("KRW-BCHA", bcha*0.9995)
                post_message(myToken,"#amm", "BCHA sell: 장마감")
