...
print("VET-autotradestart")
# 시작 메세지 슬랙 전송
post_message(myToken,"#amm", "VET-autotradestart")
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=120):
            # 코인종류 & k 값 설정
            target_price = get_target_price("KRW-VET", 0.4)
            btc = get_balance("VET")
            krw = get_balance("KRW")
            current_price = get_current_price("KRW-VET")
            yesterday_price = get_yesterday_price("KRW-VET")
            ...
            if target_price*1.20 < current_price:
                if btc*current_price > 100000:
                    sell_result = upbit.sell_market_order("KRW-VET", btc*0.25)
    ...
    #에러메세지 출력
        ...
        print(e)
        post_message(myToken,"#amm", "VET error")
