...
print("DOT-autotradestart")
# 시작 메세지 슬랙 전송
post_message(myToken,"#amm", "DOT-autotradestart")
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=120):
            ...
            yesterday_price = get_yesterday_price("KRW-DOT")
            ...
            if target_price < current_price and 10000 > btc*current_price:
                if krw > 202000:
                    ...
                    post_message(myToken,"#amm", "DOT buy : 타겟 구매")

            #타겟 가격보다 20%이상 상승 시, 10만원 밑으로 남기고 25%씩 매도 (마이 묵었다)
            if target_price*1.20 < current_price:
                if btc*current_price > 100000:
                    sell_result = upbit.sell_market_order("KRW-DOT", btc*0.25)
                    post_message(myToken,"#amm", "DOT sell: 익절 흐믓")
            ...
            if current_price < yesterday_price*0.92 and current_price > yesterday_price*0.82 and 10000 > btc*current_price:
                if krw > 101000:
                    ...
                    post_message(myToken,"#amm", "DOT buy : 가즈아 구매")

            #어제 가격보다 20%이상 하락 시, 다팔아
            if current_price < yesterday_price*0.8:
                if btc*current_price > 5000:
                    sell_result = upbit.sell_market_order("KRW-DOT", btc*0.9995)
                    post_message(myToken,"#amm", "DOT sell all: 망했어요")
        ...
        #장마감 - 가진거 다팔아
            ...
            if btc*current_price > 5000:
                sell_result = upbit.sell_market_order("KRW-DOT", btc*0.9995)
                post_message(myToken,"#amm", "DOT sell all : 장마감")
        time.sleep(1)
    
    #에러메세지 출력
    except Exception as e:
        print(e)
        post_message(myToken,"#amm", "DOT error : " +e)
        time.sleep(1)