...
print("BTT_autotrade start")
# 시작 메세지 슬랙 전송
post_message(myToken,"#amm", "BTT_autotrade start")
    ...
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if target_price < current_price and ma15 < current_price:
                krw = get_balance("KRW")
                if krw > 5000:
                    buy_result = upbit.buy_market_order("KRW-BTT", krw*0.49975)
                    post_message(myToken,"#amm", "BTT buy : " +str(buy_result))
        else:
            btt = get_balance("BTT")
            if btt > 0.00008:
                sell_result = upbit.sell_market_order("KRW-BTT", btt*0.9995)
                post_message(myToken,"#amm", "BTT buy : " +str(sell_result))
        time.sleep(1)
